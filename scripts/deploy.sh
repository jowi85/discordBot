...
kubectl config set users.fp-discord-bot.user.token "$KUBE_SVC_ACCT_TOKEN"

# kubectl set image deployment/fp-discord-bot fp-discord-bot=docker.io/cronym/fp-discord-bot:"${TRAVIS_TAG}"
echo "kubectl set image deployment/fp-discord-bot fp-discord-bot=docker.io/cronym/fp-discord-bot:\"${TRAVIS_TAG}\""
